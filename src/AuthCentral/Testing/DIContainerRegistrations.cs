...
namespace Fsw.Enterprise.AuthCentral.Testing
{
    ...
    {
        public static void ConfigureTestUserService(this IdentityServerServiceFactory factory, IApplicationBuilder app, IApplicationEnvironment appEnvironment,
            EnvConfig config)
        {
            factory.Register(new Registration<UserAccount, HierarchicalUserAccount>());
            factory.Register(
                new Registration<MembershipRebootConfiguration<HierarchicalUserAccount>>(
                    x => MembershipRebootConfigFactory.GetDefaultConfig(appEnvironment, x.Resolve<ILoggerFactory>(), config)));
        ...
        {
            services.AddScoped<MembershipRebootConfiguration<HierarchicalUserAccount>>(
                provider => MembershipRebootConfigFactory.GetDefaultConfig(env, provider.GetService<ILoggerFactory>(), config));
